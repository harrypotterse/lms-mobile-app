...
class SearchPageRepository {
  static Future<ApiResponse<SearchPageResponse>> getSearchPageRepositoryData(
      String? globalSearch) async {
    try {
      EasyLoading.show(status: 'loading...');
      var response =
          await ApiService.getDio()!.get("/courses");
      ...
      if (response.statusCode == 200) {
        ...
        var obj = searchPageResponseFromJson(response.toString());
        return ApiResponse(
      ...
      if (e.type == DioExceptionType.unknown) {
        EasyLoading.dismiss();
        var obj = searchPageResponseFromJson(e.response.toString());
