...
class NotificationRepository {
  static Future<ApiResponse<NotificationResponse>> getNotificationRepositoryData() async {
    try {
      EasyLoading.show(status: 'loading...');
      var response = await ApiService.getDio()!.get("/notifications");
      EasyLoading.dismiss();
      if (response.statusCode == 200) {
        if (kDebugMode) {
          print(response.data);
        }
        var obj = notificationResponseFromJson(response.toString());
        return ApiResponse(
      ...
      if (e.type == DioExceptionType.unknown) {
        EasyLoading.dismiss();
        var obj = notificationResponseFromJson(e.response.toString());
